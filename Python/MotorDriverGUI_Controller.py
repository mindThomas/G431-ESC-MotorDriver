...
class Toolbar(Tk.Frame):
    def __init__(self, parent):
        ...
        SetCurrentKpTiButton = Tk.Button(Horizontal3, text='Set Kp/Ti', command=lambda: self.__btnHandler__('set_kpti'))
        ...
        StartCurrentSweepButton = Tk.Button(Horizontal2, text='Current', command=lambda: self.__btnHandler__('start_current_sweep'))
...
def ControllerDebugCallback(data, params):
    global previous_time
    global previous_encoder
    CurrentSetpoint, CurrentFiltered, OmegaFiltered, IntegralTerm, PI, Duty, csv_obj = params

    single_sample_package_length = 36
    if len(data) == single_sample_package_length:
        [timestamp, current_setpoint, current_measurement, current_filtered, omega_measurement, omega_filtered, integral, PIout, duty_set] = struct.unpack('<Iffffffff', data)
        timestamp = float(timestamp) / 100000
        ...
        if csv_obj:
            csv_obj.write_row([timestamp, float(current_setpoint), float(current_measurement), float(current_filtered), float(omega_measurement), float(omega_filtered), float(integral),
                               float(PIout), float(duty_set)])
...
def main():
    ...
    com.registerCallback(0x05, ControllerDebugCallback, (CurrentSetpoint, CurrentFiltered, OmegaFiltered, Integral, PIout, DutyCycle, logger))
    ...
    main.toolbar.registerCallback('set_kpti', SetCurrentKpTi, (com, main.toolbar.CurrentKp, main.toolbar.CurrentTi))
