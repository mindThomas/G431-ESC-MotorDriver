...
class CANBus:
    ...
    def open(self):
        ...
        # InitCAN
        ret = self.dll.VCI_InitCAN(
            self.nDeviceType, self.nDeviceInd, self.nCANInd, byref(self.config)
        )
    ...
    def available(self):
        ret = self.dll.VCI_GetReceiveNum(
            self.nDeviceType, self.nDeviceInd, self.nCANInd
        )
    ...
    def transmit(self, ID, data):
        ...
        ret = self.dll.VCI_Transmit(
            self.nDeviceType, self.nDeviceInd, self.nCANInd, byref(txPackage), 1
        )
        return ret
