function [X_out, P_out] = MotorCurrentEKF(X, P_prev, ...
    ...
    i_prev = X(1); % average current posterior estimate from previous timestep
    i0_prev = X(2); % bottom/minimum current posterior estimate from previous timestep
...
%     
%     X_apriori(1) = i_apriori;
%     X_apriori(2) = i0_apriori;
%     
%     % Determine model Jacobian (F)   -  OBS. This is not supposed to use/depend on apriori states!
%     F_prev = single(zeros(2,2));
%     F_prev(1,1) = 0; % di_apriori / di_prev
%     F_prev(1,2) = 1/dt*L/R*(1-exp(-R/L*dt)); % di_apriori / i0_prev
%     F_prev(2,1) = 0; % di0_apriori / di_prev
%     F_prev(2,2) = exp(-R/L*dt); % di0_apriori / i0_prev
% 
...
%     P_apriori = F_prev * P_prev * F_prev' + Q_proc;           
    ...
    % Compute internal estimates using apriori
    i_max = (i0_apriori - Vin/R)*exp(-R/L*tON) + Vin/R;
    i_min = i_max * exp(-R/L*tOFF);
    ...
    for (i = 1:size(i_meas, 1))        
        ...
        if (z_sample_duty < duty)
            ...
            iON = (i0_apriori - Vin/R) * exp(-R/L*t_sample) + Vin/R;
            diON_di0_apriori = exp(-R/L*t_sample);
            
            z_hat(i,1) = iON;
            H(i,:) = diON_di0_apriori;
        else
    ...
    P_aposteriori = (1 - K*H) * P_apriori;     
     
    %% Compute output based on prediction model
    i0_aposteriori = X_aposteriori;
    
    % Prediction step - use the current models derived in OneNote
    i_avg = 1/dt * (Vin/R*tON + L/R*i0_aposteriori*(1-exp(-R/L*dt)) + L/R*Vin/R*exp(-R/L*dt) - L/R*Vin/R*exp(-R/L*tOFF)); % apriori estimate of average current    
    i_max = (i0_prev - Vin/R)*exp(-R/L*tON) + Vin/R;
    i_min = i_max * exp(-R/L*tOFF);
