% Based on a modified version of RL_EKF2 where resistance estimation is
% stripped out
function [X_out, P_out, i_avg] = L_EKF(X, P, ...
    ...
    Q_proc = diag([0, 0.000000001]); % process noise
    ...
    if (location_sample1 > 0 && location_sample1 <= duty)
        ...
        H(1,:) = [di_sample1_di0_prev, di_sample1_dL];
    else
    ...
    if (location_sample2 > 0 && location_sample2 > duty)
        ...
        H(2,:) = [di_sample2_di0_prev, di_sample2_dL];              
    else
    ...
    F(1,2) = di_max_dL*exp(-R/L*tOFF) + R/L^2*tOFF*(i_max + Ke*omega/R)*exp(-R/L*tOFF); % di0_next / dL    
    F(2,1) = 0; % dL / di0_aposteriori
    F(2,2) = 1; % dL / dL
