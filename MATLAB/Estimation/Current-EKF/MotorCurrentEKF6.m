function [X_out, P_out, i_avg] = MotorCurrentEKF6(X, P, ...
    ...
    Q_proc = diag([0.001, 0.0001]); % process noise
    ...
    t_sample = duty/2 * dt;
    iON_MID_hat = (i0_prev - (Vin-Ke*omega)/R) * exp(-R/L*t_sample) + (Vin-Ke*omega)/R;    
    diON_di0_prev = exp(-R/L*t_sample);
    diON_dR = (Vin-Ke*omega)/R^2*exp(-R/L*t_sample) - t_sample/L*(i0_prev - (Vin-Ke*omega)/R)*exp(-R/L*t_sample) - (Vin-Ke*omega)/R^2;
    z(1,1) = i_ON_MID;
    ...
    i_max = (i0_prev - (Vin-Ke*omega)/R) * exp(-R/L*tON) + (Vin-Ke*omega)/R;    
    ...
    iOFF_MID_hat = (i_max + Ke*omega/R) * exp(-R/L*t_sample) - Ke*omega/R;
    diOFF_di_max = exp(-R/L*t_sample);                            
    diOFF_di0_prev = diOFF_di_max * di_max_di0_prev;
    diOFF_dR = (di_max_dR - Ke*omega/R^2)*exp(-R/L*t_sample) - t_sample/L*(i_max + Ke*omega/R)*exp(-R/L*t_sample) + Ke*omega/R^2;
